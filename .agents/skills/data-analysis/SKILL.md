...
<!--
PILOT / UNVALIDATED (Antigravity mirror of .claude/skills/data-analysis/SKILL.md,
Sept 2026). This stage is pure Python-script orchestration with no agent-specific
mechanics, so it's the least likely of the four to need correction -- but it still
hasn't been run inside Antigravity. See .agents/PILOT-NOTES.md.
-->
...
Confirm `projects/<slug>/04_synthetic_test/subjects_data.csv`, `population_config.json`, and `test_config.json` exist. If missing, point the user to the synthetic-ab-test skill rather than reconstructing them.
