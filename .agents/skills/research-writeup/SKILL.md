...
<!--
PILOT / UNVALIDATED (Antigravity mirror of .claude/skills/research-writeup/SKILL.md,
Sept 2026). Pure Python/pandoc orchestration, no agent-specific mechanics -- but
still unrun inside Antigravity. See .agents/PILOT-NOTES.md.
-->
...
Confirm `projects/<slug>/03_design/`, `04_synthetic_test/`, and `05_analysis/` all exist and are populated (balance report, regression results, charts). If Stage 5 hasn't been run, point the user to the data-analysis skill rather than improvising numbers.

Confirm `pandoc` is on PATH (`pandoc --version`); if missing, install it (winget on Windows, Homebrew on macOS — see the workbook's Section 1.5 for exact commands) before proceeding.
