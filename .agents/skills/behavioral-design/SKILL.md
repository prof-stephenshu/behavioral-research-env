...
<!--
PILOT / UNVALIDATED (Antigravity mirror of .claude/skills/behavioral-design/SKILL.md,
Sept 2026). Domain logic below is unchanged from the tested Claude Code version.
See .agents/PILOT-NOTES.md before relying on this for a real run.
-->
...
**`stimuli/control.md`, `stimuli/treatment.md`** — the actual stimulus content as given by the user. If the stimulus is a visual artifact (landing page, popup, email, ad, physical mailer) and your environment supports rendering a self-contained HTML mockup for the user to review, do that (favicon: a single relevant emoji, e.g. 🧪) — otherwise a plain description is fine. Keep it faithful to what the user described — don't invent copy or layout details they didn't give you.
...
After producing the package, summarize it back to the user in a few lines (conditions, principle, population summary, outcome variable) and explicitly ask them to confirm before proceeding to Stage 4. Do not run the synthetic-ab-test skill until they confirm. If they want changes, edit the relevant file(s) and re-summarize.
